...
static int SENSITIVITY = 1000; //1gモード
...
void app_app_main(void)
{
    ...
    int step_count = 0;
    bool flag = false;
    dequeue(&cb, &value); //古い値を削除
    enqueue(&cb, mag); //新しい値を追加
    sum = sum - value + mag;
    ave = sum / BUFFER_SIZE; //平均を計算
    diff = abs(ave - mag);
    if(diff > SENSITIVITY && !flag){
        flag = true;
        step_count ++;
    }
    if(diff < SENSITIVITY && flag){
